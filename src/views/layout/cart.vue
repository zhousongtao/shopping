   ...
   <div v-if="isLogin && cartList.length > 0" >
      ...
      <div class="cart-item" v-for="item in cartList" :key="item.goods_id">
        ...
        <div class="info">
          ...
          <span class="bottom">
            ...
            <!-- 既希望保留原本的形参，又需要通过调用函数传参 可以使用箭头函数包装一层 -->
    ...
    <div class="footer-fixed">
      ...
      <div class="all-total">
        ...
        <div v-else class="delete" :class="{disabled:selCount === 0 }" @click="delItem">删除</div>
      </div>
    </div>
   </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('cart', ['cartTotal', 'selPrice', 'selCount', 'isAllchecked', 'selCartList']),
    isLogin () {
  ...
  methods: {
    toggleCheck (id) {
      this.$store.commit('cart/changeItemChecked', id)
    },
    toggleAllCheck () {
    ...
    changeCount (goodsNum, goodsId, goodsSkuId) {
      ...
      this.$store.dispatch('cart/changeItemCount', { goodsNum, goodsId, goodsSkuId })
    },
    delItem () {
  ...
  created () {
    ...
    if (this.isLogin) {
      ...
      this.$router.push('/login?backUrl=cart')
    }
  }
}
</script>
