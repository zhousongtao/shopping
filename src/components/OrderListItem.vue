  ...
  <div class="order-list-item" v-if="item.order_id">
    ...
    <div class="actions">
      <div v-if="item.order_status === 10">
        <span v-if="item.pay_status === 10"  @click="routineConfirm">立刻付款</span>
        <span v-else-if="item.delivery_status === 10"  @click="routineConfirm">申请取消</span>
        <span v-else-if="item.delivery_status === 20 || item.delivery_status === 30"  @click="routineConfirm">确认收货</span>
      </div>
